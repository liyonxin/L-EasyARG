...
def create_default_config(output_file="config.yaml"):
    """Create a default configuration file"""
    config = load_config()
    try:
        with open(output_file, 'w') as f:
            yaml.dump(config, f, default_flow_style=False)
        logger.info(f"Created default configuration file at {output_file}")
    except Exception as e:
...
Examples:
  ...
  easy-arg plot --type top10 --sample SRR123456
    ...
    init_parser.add_argument("--output", "-o", default="config.yaml", help="Output file name")
    ...
    if args.command == "run":
        ...
        if not args.sample:
            args.sample = Path(args.input).stem.replace(".fastq.gz", "").replace(".fq.gz", "").replace(".fastq", "").replace(".fq", "")
        ...
        try:
            ...
            run_analysis_pipeline(
                input_path=os.path.abspath(os.path.join(original_cwd, args.input)),
